...
if test ! -d {{ venvName }}; then
    ...
    # Bootstrap it
    {{ pythonForVenv }} -m pip install pip wheel --upgrade
    ...
    echo ' ' >> {{ venvName }}/Scripts/activate
    echo 'export ARGCOMPLETE_USE_TEMPFILES=1' >> {{ venvName }}/Scripts/activate
    echo 'eval "$(register-python-argcomplete nmk)"' >> {{ venvName }}/Scripts/activate
fi
